...
export default class ResourcesLoader extends EventEmitter {
  ...
  private sourceLoaded(source: Sources, file: any) {
    ...
    if (source.groupName) {
      this.groupedItems[source.groupName].push({
        [source.name]: file,
        name: source.name,
        type: source.type,
      });
      if (
        this.groupedItems[source.groupName].length === source.totalGroupMember
      ) {
  ...
  destroy() {
    // dispose all created textures and gltfModel
            ...
            file.scene.traverse((child: any) => {
              if (child instanceof THREE.Mesh) {
                ...
                for (const key in child.material) {
                  if (
                    Object.prototype.hasOwnProperty.call(child.material, key)
                  ) {
    ...
    for (const key in this.items) {
      if (Object.prototype.hasOwnProperty.call(this.items, key)) {
        ...
        if (
          ...
          file instanceof THREE.CubeTexture
        ) {
          if (typeof file.dispose === "function") file.dispose();
        } else {
          // treat it as a gltf model
          file.scene.traverse((child: any) => {
            if (child instanceof THREE.Mesh) {
              if (typeof child.geometry.dispose === "function")
                child.geometry.dispose();
