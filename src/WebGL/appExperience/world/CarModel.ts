...
export default class CarModel {
  ...
  init() {
    this.resourceLoader.on("carModelGroupReady", () => {
      // ----- Car model is done loading ------
  ...
  transitionCompactmentIn() {
    // ------ animate car compactment into camera view ----------
    const tyreVectors = this.compactments.carTyres.map((tyre) => tyre.position);
    const doorsVectors = this.compactments.carDoors.map(
      (door) => door.position
    );
    const screenVectors = this.compactments.carScreenGlasses.map(
      (screen) => screen.position
    );
    const chairVectors = this.compactments.carChairs.map(
    ...
    if (this.compactments.carBody) {
      tl.to(tyreVectors, {
        ...
        .to(
          [
            this.compactments.carShadowMesh?.position,
            this.compactments.carBottom?.position,
            this.compactments.carRoadLaneMesh?.position,
          ],
          {
            y: `-=${this.compactments.positionOffset}`,
          }
  ...
  setUpFloor() {
    // -------- set up car floor shadow -----------
    // --------- setup texture -------------
    const floorTexture = this.resourceLoader.items
      .carFloorTexture as THREE.Texture;
  ...
  setUpRoadLane() {
    //--------- setup texture----------
    const laneTexture = this.resourceLoader.items
      .carLaneTexture as THREE.Texture;
  ...
  addDebugUi() {
    ...
    if (this.debugUi.isActive && this.debugUi.ui) {
      const carFolder = this.debugUi.ui.addFolder({
        title: "carModel",
        expanded: false,
      });
      ...
      if (this.car.mesh) {
        const carParams = {
          ...
          rotation: {
            x: this.car.mesh.rotation.x,
            y: this.car.mesh.rotation.y,
            z: this.car.mesh.rotation.z,
          },
          scale: {
            x: this.car.mesh.scale.x,
            y: this.car.mesh.scale.y,
            z: this.car.mesh.scale.z,
          },
        };
  ...
  update() {
    if (this.car.mesh) {
      //------------make camera look at car on each tick------------
    ...
    if (this.car.isAccelerating) {
      ...
      this.compactments.carTyres.forEach((tyre) => {
        tyre.rotation.x += this.time.deltaTime * 0.1;
      });
      ...
      if (
        this.compactments.carRoadLaneMesh!.position?.z <
        this.roadLaneAnimation.endThreshold
      ) {
        this.compactments.carRoadLaneMesh!.position.z =
          this.roadLaneAnimation.startThreshold;
      }
    }
  }
}
