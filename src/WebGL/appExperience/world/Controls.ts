...
export default class Controls {
  ...
  disableCameraOnCurveView() {
    ...
    tl.to(this.lerp, {
      ...
      duration: 2,
      onComplete: () => {
        this.camera.isDisableOnCurve = true;
      },
    })
  ...
  addDebugUI() {
    if (this.debugUI.isActive && this.debugUI.ui) {
      const controlFolder = this.debugUI.ui.addFolder({
        title: "Controls",
        expanded: true,
      });
      const controlsParams = {
        curvePosition: {
          x: this.curve.mesh.position.x,
          y: this.curve.mesh.position.y,
          z: this.curve.mesh.position.z,
        },
        curveRotation: {
          x: this.curve.mesh.rotation.x,
          y: this.curve.mesh.rotation.y,
          z: this.curve.mesh.rotation.z,
        },
      };

      controlFolder
        .addInput(controlsParams, "curvePosition")
        .on("change", () => {
      ...
      controlFolder
        .addInput(controlsParams, "curveRotation")
        .on("change", () => {
